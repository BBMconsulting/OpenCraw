...
describe("config view", () => {
  ...
  it("keeps a root-level unsupported schema unsafe in a scoped section", () => {
    const rendered = renderConfigView({
      schema: {
        anyOf: [
          {
            type: "object",
    ...
    expect(normalizedText(banner)).toContain(
      "1 setting in this config can only be edited as text: <root>",
    );
    expect(findButtonByText(rendered.container, "Form").getAttribute("title")).toBe(
      "Form view can't safely edit some fields",
    );
  });
