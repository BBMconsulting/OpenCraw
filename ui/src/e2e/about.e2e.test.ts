...
describeControlUiE2e("Control UI About mocked Gateway E2E", () => {
  ...
  it("shows and copies browser artifact identity, separately from the Gateway version", async () => {
    ...
    try {
      ...
      // CRAWCRAW_GREETING_MS clears the class after 1400ms, so click and read it in one browser step.
      const crawCrawWaving = await crawCraw.evaluate(async (element) => {
        ...
        return button.classList.contains("about-hero__crawCraw--wave");
      });
      expect(crawCrawWaving).toBe(true);
