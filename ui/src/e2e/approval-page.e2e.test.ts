...
describeControlUiE2e("Control UI standalone approval page", () => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — OpenCraw");
      ...
      expect(await mobile.page.title()).toBe("Approved here — OpenCraw");
      await waitForStableApprovalPaint(mobile.page);
      await mobile.page.screenshot({
      ...
      expect(await mobile.page.title()).toBe("Approved — OpenCraw");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
  ...
  it("retains context but disables decisions through a reconnect", async () => {
    ...
    await surface.page
      .getByText(
        "OpenCraw cannot confirm or record a decision while disconnected. Reconnect to check the current status.",
        { exact: true },
      )
      .waitFor();
