...
function themeCard(page: Page, theme: "craw" | "knot") {
  return page.locator(`.settings-theme-card--${theme}`);
}

async function expectThemeActive(page: Page, theme: "craw" | "knot"): Promise<void> {
  await expect
...
describeControlUiE2e("Control UI server prefs reconnect sync", () => {
  ...
  it("replays an offline theme edit after a same-client reconnect", async () => {
    ...
    const initial = configResponse({ theme: "craw", themeMode: "system" }, "prefs-a-1");
    ...
    try {
      ...
      await themeCard(page, "craw").waitFor();
  ...
  it("keeps disjoint edits from two pages after both hash-free patches", async () => {
    ...
    const initial = configResponse({ theme: "craw" }, "prefs-b-1");
    ...
    try {
      ...
      await Promise.all([themeCard(pageA, "craw").waitFor(), themeCard(pageB, "craw").waitFor()]);
  ...
  it("applies a server delta without reverting a pending local key", async () => {
    const context = await createContext();
    const page = await context.newPage();
    const initial = configResponse({ locale: "en", theme: "craw" }, "prefs-c-1");
    const gateway = await installMockGateway(page, {
      methodResponses: { "config.get": initial },
    });
    ...
    try {
      ...
      const serverChanged = configResponse({ locale: "de", theme: "craw" }, "prefs-c-2");
