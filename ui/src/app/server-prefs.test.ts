...
describe("pushServerUiPrefs", () => {
  ...
  it("serializes a reconnect flush behind the active write", async () => {
    const first = deferred();
    let requestCalls = 0;
    const request = vi.fn<(method: string, params?: unknown) => Promise<unknown>>(() => {
      requestCalls += 1;
      return requestCalls === 1 ? first.promise : Promise.resolve({});
    });
    const client = createClient(request);
  ...
  it("serializes a replacement client behind the prior client's active write", async () => {
    const first = deferred();
    const priorRequest = vi.fn<(method: string, params?: unknown) => Promise<unknown>>(
      () => first.promise,
    );
    const replacementRequest = vi.fn<(method: string, params?: unknown) => Promise<unknown>>(
      async () => ({}),
    );

    pushServerUiPrefs(createClient(priorRequest), { themeMode: "dark" });
    await vi.waitFor(() => expect(priorRequest).toHaveBeenCalledOnce());
    pushServerUiPrefs(createClient(replacementRequest), { themeMode: "light" });

    expect(replacementRequest).not.toHaveBeenCalled();
    first.resolve({});

    await vi.waitFor(() => expect(replacementRequest).toHaveBeenCalledOnce());
    expect(replacementRequest.mock.calls[0]?.[1]).toEqual({
      raw: JSON.stringify({ ui: { prefs: { themeMode: "light" } } }),
      note: "control-ui prefs sync",
    });
    await vi.waitFor(() => expect(localStorage.getItem(pendingKey("ws://gw"))).toBeNull());
  ...
  it("retains a transient connected failure and replays it on flush", async () => {
    const request = vi
