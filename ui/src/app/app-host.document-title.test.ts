...
describe("OpenCraw shell document title", () => {
  ...
  it("keeps the boot title before a route commits", () => {
    ...
    document.title = "OpenCraw Control";

    shell.routeState = {};
    shell.syncDocumentTitle();
    expect(document.title).toBe("OpenCraw Control");
  });
  ...
  it("uses the route title for a connected route", () => {
    ...
    shell.syncDocumentTitle();
    expect(document.title).toBe("Usage — OpenCraw");
  });
  ...
  it("uses the active session's derived title for a non-main chat", () => {
    ...
    expect(document.title).toBe("Quarterly launch plan — OpenCraw");
  });
  ...
  it("uses the agent name for an agent main chat", () => {
    ...
    expect(document.title).toBe("Molty — OpenCraw");
  });
  ...
  it("falls back to the session display name when the main agent is missing", () => {
    ...
    expect(document.title).toBe("Fallback thread — OpenCraw");
  });
  ...
  it("prefixes the pending approval count", () => {
    ...
    expect(document.title).toBe("(2) Usage — OpenCraw");
  });
  ...
  it("shows offline instead of a stale approval count", () => {
    ...
    expect(document.title).toBe("(Offline) Usage — OpenCraw");
  });
  ...
  it("includes stored chat outbox messages in the offline marker", () => {
    ...
    expect(document.title).toBe("(Offline · 3 queued) Usage — OpenCraw");
  });
  ...
  it("uses the meaningful custodian label without a brand suffix", () => {
    ...
    expect(document.title).toBe("Ask OpenCraw");
  });
});
