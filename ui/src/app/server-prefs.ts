...
function isPermanentPrefsWriteError(error: unknown): boolean {
  return (
  ...
  while (pendingPrefs) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        if (!isPermanentPrefsWriteError(error)) {
          // Unknown, timeout, transport, and explicitly retryable failures retain intent.
          // A later reconnect flush or local edit will drain it again.
          return;
        }
        // Proven validation or viewer-scope rejection degrades to device-local state.
        removeBatch(batch);
        settlePendingStorage(batch);
        return;
      }
