...
describe("formatDocumentTitle", () => {
  it("suffixes the brand after a plain context", () => {
    expect(formatDocumentTitle({ context: "Usage" })).toBe("Usage — OpenCraw");
  });

  it("does not duplicate a context ending in the brand", () => {
    expect(formatDocumentTitle({ context: "Ask OpenCraw" })).toBe("Ask OpenCraw");
    expect(formatDocumentTitle({ context: "OpenCraw" })).toBe("OpenCraw");
  });
  ...
  it("includes the queued outbox count in the offline marker", () => {
    expect(formatDocumentTitle({ context: "Usage", offline: true, queuedCount: 3 })).toBe(
      "(Offline · 3 queued) Usage — OpenCraw",
    );
  });

  it("ignores a queued count while online", () => {
    expect(formatDocumentTitle({ context: "Usage", queuedCount: 3 })).toBe("Usage — OpenCraw");
  });
