...
import { scheduleStaleChunkReloadForImportError } from "../app/stale-chunk-reload.ts";
...
class AppSidebar extends AppSidebarSessionNavigationElement implements SessionListHost {
  ...
  private readonly narrationImport = createIdleImport(
    () => import("./app-sidebar-session-narration.ts"),
  );
  ...
  private ensureNarrationController(): void {
    ...
    this.narrationLoad = this.narrationImport
      .load()
      .then((module) => {
        // The element may have left the DOM while the chunk loaded.
        if (!this.isConnected || this.narration) {
          return;
        }
      ...
      .catch((error: unknown) => {
        if (!scheduleStaleChunkReloadForImportError(error)) {
          console.error("[openclaw] failed to load sidebar narration", error);
        }
      })
      .finally(() => {
