# OpenCraw Control UI Design System

OpenCraw's default `Craw` theme is a **dark-first, glass-surface system** built
around a deep charcoal base (`#0e1015`), the production wordmark blue
(`#0088ff`), and layered frosted-glass surfaces that create depth without solid
panels. Its cool light mode uses `#f7faff` surfaces with a contrast-adjusted
`#0068c9` accent. Motion is crisp and purposeful—fast micro-interactions
(100ms) with spring-loaded expansions. Interactive text, controls, borders, and
focus treatment meet the applicable WCAG 2.1 AA contrast thresholds.
...
3. **Accent with restraint** — OpenCraw blue (`--accent: #0088ff`) for primary actions; sky blue (`--accent-2`) for secondary emphasis. Red remains reserved for semantic danger and destructive states.
