...
export async function buildOnboardingWelcome(params: {
  ...
  if (!defaultModel) {
    throw new Error(
      "OpenCraw onboarding requires working inference first. Run `openclaw onboard` to configure and verify a default model.",
    );
  }
  ...
  const welcome = [
    "## Hi, I'm OpenCraw — let's hatch your agent.",
    "",
