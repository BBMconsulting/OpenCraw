...
export function formatSystemAgentOverview(overview: SystemAgentOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "OpenCraw online. Little claws, typed tools.",
    "",
    `Config: ${configStatus}`,
    ...
    `AI: ${
      overview.defaultModel
        ...
        : "inference unavailable; run openclaw onboard before starting OpenCraw"
    }`,
...
export function formatSystemAgentStartupMessage(overview: SystemAgentOverview): string {
  ...
  return [
    "Hi, I'm OpenCraw — caretaker of this gateway, config, channels, and agents.",
