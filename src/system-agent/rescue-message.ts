// OpenCraw rescue messages expose approved setup-helper commands over message channels.
...
function createCaptureRuntime(): { runtime: RuntimeEnv; read: () => string } {
  ...
  return {
    runtime: {
      ...
      error: push,
      exit: (code) => {
        throw new Error(`OpenCraw operation exited with code ${code}`);
      },
    },
    read: () => lines.join("\n").trim(),
  };
...
function formatUnsupportedRemoteOperation(operation: SystemAgentOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      "OpenCraw rescue cannot open the local TUI from a message channel.",
  ...
  if (operation.kind === "channel-setup") {
    return [
      "OpenCraw rescue cannot host the interactive channel setup from a message channel.",
      "Run `openclaw setup` locally and say `connect " + operation.channel + "` instead.",
    ].join(" ");
  }
  if (operation.kind === "model-setup") {
    return [
      "OpenCraw rescue cannot host model-provider credential setup from a message channel.",
      "Run `openclaw onboard` locally; it live-tests the candidate route before saving it.",
    ].join(" ");
  }
  if (operation.kind === "doctor-fix") {
    return [
      "OpenCraw rescue cannot run doctor repairs from a message channel because they can change the inference route powering this session.",
      "Exit OpenCraw and run `openclaw doctor --fix` in a terminal.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      "OpenCraw rescue cannot install plugins from a message channel by default because plugin install downloads executable code.",
...
export async function runSystemAgentRescueMessage(
  ...
  if (approvalIntent === "approve") {
    ...
    if (!operation) {
      return "No pending OpenCraw rescue change is waiting for approval.";
    }
    ...
    return capture.read() || "OpenCraw rescue change applied.";
  }

  if (approvalIntent === "decline") {
    const pending = parsePendingOperation(pendingStore.consume(pendingKey));
    return pending
      ? "Dropped the pending OpenCraw rescue change."
      : "No pending OpenCraw rescue change is waiting for approval.";
  }
  ...
  if (isPersistentSystemAgentOperation(operation)) {
    ...
    if (nowMs === undefined || expiresAtMs === undefined) {
      return "OpenCraw rescue could not create a pending approval because the expiry clock is invalid.";
    }
  ...
  return capture.read() || "OpenCraw listened, clicked a claw, and found nothing to change.";
}
