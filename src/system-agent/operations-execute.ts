...
export async function executeSystemAgentOperation(
  ...
  switch (operation.kind) {
    ...
    case "model-setup":
      runtime.log(
        [
          "Changing model providers must happen outside the inference session that powers OpenCraw.",
          "Exit OpenCraw and run `openclaw onboard`; it stages credentials, live-tests the candidate route, and saves only a passing setup.",
        ].join("\n"),
      );
      return { applied: false };
    case "open-setup": {
    ...
    case "plugin-uninstall": {
      if (await isPluginBackingDefaultInferenceRoute(operation.pluginId)) {
        const message = [
          `Uninstalling ${operation.pluginId} could remove the provider behind OpenCraw's own active inference route.`,
          `Exit OpenCraw and run \`openclaw plugins uninstall ${operation.pluginId}\` from a terminal.`,
        ].join("\n");
        runtime.log(message);
      ...
      const result = await applyPersistentOperation({
        ...
        run: async (ctx) => {
          ...
          await ctx.commit(async () => {
            ...
            if (await isPluginBackingDefaultInferenceRoute(operation.pluginId)) {
              throw new Error(
                `Uninstall aborted: ${operation.pluginId} now backs the active inference route. Exit OpenCraw and run \`openclaw plugins uninstall ${operation.pluginId}\` from a terminal.`,
              );
            }
    ...
    case "create-agent": {
      ...
      if (operation.model?.trim()) {
        throw new Error(
          "OpenCraw cannot save an explicit per-agent model until that new route can be live-tested. Retry without `model`; the new agent inherits the verified default, then use `set_default_model` with agentId to live-test and save its own model.",
        );
      }
      return await applyPersistentOperation({
    ...
    case "doctor-fix":
      runtime.log(
        "Doctor repairs can change the inference route that powers this session. Exit OpenCraw and run `openclaw doctor --fix` in a terminal.",
      );
      return { applied: false };
    case "status": {
