...
export async function applyPersistentOperation(params: {
  ...
  try {
    ...
    runtime.error(
      `${outcome.summary}, but OpenCraw could not record its audit entry: ${formatErrorMessage(error)}`,
    );
  }
...
export async function assertConfigWriteDoesNotBypassInferenceVerification(
  ...
  if (verdict === "plugin-entry") {
    ...
    throw new Error(
      `Direct config writes cannot change plugin "${pluginId}" because it may back OpenCraw's own active inference route. Exit OpenCraw and edit it from a terminal.`,
    );
  }
  ...
  throw new Error(
    denialReason
      ...
      : "Direct config writes cannot change the default inference route or include alternate config. Use `set_default_model` (optionally with agentId) for an already configured route, or exit OpenCraw and run `openclaw onboard` to change provider/auth access.",
  );
}
...
async function verifyCurrentSetupInference(
  ...
  if (!before.exists || !before.valid) {
    throw new Error(
      "OpenCraw setup requires a valid configured inference route. Exit OpenCraw and run `openclaw onboard`, then retry.",
    );
  }
  ...
  if (!beforeRoute.route) {
    throw new Error(
      "OpenCraw setup requires working inference first. Exit OpenCraw and run `openclaw onboard`, then retry.",
    );
  }
  ...
  if (!verification.ok) {
    throw new Error(
      `OpenCraw setup requires working inference first. The configured route failed a live check: ${verification.error} Exit OpenCraw and run \`openclaw onboard\`, then retry.`,
    );
  }
...
export async function executeSetup(
  ...
  if (!defaultModel) {
    throw new Error(
      "OpenCraw setup requires working inference first. Run `openclaw onboard` to configure and verify a default model, then start OpenCraw again.",
    );
  }
  const requestedModel = operation.model?.trim();
  if (requestedModel && requestedModel !== defaultModel) {
    throw new Error(
      `OpenCraw setup will preserve the verified default model ${defaultModel}. Exit OpenCraw and run \`openclaw onboard\` to stage, live-test, and save a different inference route.`,
    );
  }
  if (!opts.approved) {
    const message = [
  ...
  if (requestedModel && requestedModel !== verified.modelRef) {
    throw new Error(
      `The verified default model is now ${verified.modelRef}, not ${requestedModel}. Review the current route or exit OpenCraw and run \`openclaw onboard\` before retrying setup.`,
    );
  }
