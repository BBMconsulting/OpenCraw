...
export function validateSystemAgentPluginInstallSpec(spec: string): string | null {
  ...
  if (/\s/.test(trimmed)) {
    return "OpenCraw plugin install accepts one npm or ClawHub package spec.";
  }
  if (/^(?:\.{1,2}\/|\/|~\/|file:|git(?:\+ssh|\+https)?:|https?:)/i.test(trimmed)) {
    ...
    return "OpenCraw plugin install accepts npm or ClawHub package specs only.";
  }
  if (!isOpenClawTrustedPluginInstallSpec(trimmed)) {
    return "OpenCraw installs only ClawHub, bundled, or official-catalog plugins. Use `openclaw plugins install <spec>` in a trusted shell to review an arbitrary executable source.";
  }
  return null;
}
