...
function resolveStateDirLastKnownGoodConfigPath(stateDir: string, configPath: string): string {
  const configPathHash = crypto.createHash("sha256").update(path.resolve(configPath)).digest("hex");
  return path.join(stateDir, `config.${configPathHash}.last-good`);
}
...
describe("config observe recovery", () => {
  ...
  it("derives a state-dir last-known-good path for external config paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      const lastGoodPath = resolveStateDirLastKnownGoodConfigPath(stateDir, configPath);
      deps.env.OPENCLAW_CONFIG_PATH = configPath;
  ...
  it("keeps the state-dir snapshot filename bounded for long external basenames", async () => {
    await withSuiteHome(async (home) => {
      const { deps } = makeDeps(home);
      const stateDir = path.join(home, "state");
      const configPath = path.join(home, "etc", `${"a".repeat(240)}.json`);
      const lastGoodPath = resolveStateDirLastKnownGoodConfigPath(stateDir, configPath);
      deps.env.OPENCLAW_CONFIG_PATH = configPath;
      deps.env.OPENCLAW_STATE_DIR = stateDir;
      const snapshot = await makeSnapshot(configPath, {
      ...
      expect(path.basename(lastGoodPath).length).toBeLessThanOrEqual(255);
      await expect(fsp.readFile(lastGoodPath, "utf-8")).resolves.toBe(snapshot.raw);
    });
  });

  it("derives distinct default-state-dir paths for external configs with the same basename", async () => {
    await withSuiteHome(async (home) => {
      const { deps } = makeDeps(home);
      const stateDir = path.join(home, ".openclaw");
      const configPathA = path.join(home, "etc", "a", "openclaw.json");
      const configPathB = path.join(home, "etc", "b", "openclaw.json");
      const lastGoodPathA = resolveStateDirLastKnownGoodConfigPath(stateDir, configPathA);
      const lastGoodPathB = resolveStateDirLastKnownGoodConfigPath(stateDir, configPathB);
      const snapshotA = await makeSnapshot(configPathA, {
        gateway: { mode: "local", auth: { mode: "token", token: "secret-token-a" } },
      });
      const snapshotB = await makeSnapshot(configPathB, {
        gateway: { mode: "local", auth: { mode: "token", token: "secret-token-b" } },
      });

      deps.env.OPENCLAW_CONFIG_PATH = configPathA;
      await expect(
        promoteConfigSnapshotToLastKnownGood({ deps, snapshot: snapshotA, logger: deps.logger }),
      ).resolves.toBe(true);
      deps.env.OPENCLAW_CONFIG_PATH = configPathB;
      await expect(
        promoteConfigSnapshotToLastKnownGood({ deps, snapshot: snapshotB, logger: deps.logger }),
      ).resolves.toBe(true);

      expect(lastGoodPathA).not.toBe(lastGoodPathB);
      await expect(fsp.readFile(lastGoodPathA, "utf-8")).resolves.toBe(snapshotA.raw);
      await expect(fsp.readFile(lastGoodPathB, "utf-8")).resolves.toBe(snapshotB.raw);
      await expectPathMissing(`${configPathA}.last-good`);
      await expectPathMissing(`${configPathB}.last-good`);

      const brokenRaw = "{ gateway: { mode: 123 } }\n";
      await fsp.writeFile(configPathA, brokenRaw, "utf-8");
      deps.env.OPENCLAW_CONFIG_PATH = configPathA;
      await expect(
        recoverConfigFromLastKnownGood({
          deps,
      ...
      await expect(fsp.readFile(configPathA, "utf-8")).resolves.toBe(snapshotA.raw);
      await expect(fsp.readFile(configPathB, "utf-8")).resolves.toBe(snapshotB.raw);
    });
  });

  it("migrates an authenticated legacy external-config sidecar during recovery", async () => {
    await withSuiteHome(async (home) => {
      const { deps } = makeDeps(home);
      const stateDir = path.join(home, "state");
      const configPath = path.join(home, "etc", "openclaw.json");
      const lastGoodPath = resolveStateDirLastKnownGoodConfigPath(stateDir, configPath);
      const legacyLastGoodPath = `${configPath}.last-good`;
      deps.env.OPENCLAW_CONFIG_PATH = configPath;
      deps.env.OPENCLAW_STATE_DIR = stateDir;
      const snapshot = await makeSnapshot(configPath, {
        gateway: { mode: "local", auth: { mode: "token", token: "secret-token" } },
      ...
      await fsp.rename(lastGoodPath, legacyLastGoodPath);
      ...
      await expect(fsp.readFile(configPath, "utf-8")).resolves.toBe(snapshot.raw);
      await expect(fsp.readFile(lastGoodPath, "utf-8")).resolves.toBe(snapshot.raw);
      await expect(fsp.readFile(legacyLastGoodPath, "utf-8")).resolves.toBe(snapshot.raw);
    });
  });
