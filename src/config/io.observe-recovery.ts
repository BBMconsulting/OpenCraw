...
function resolveLastKnownGoodConfigPaths(
  configPath: string,
  ...
  if (!configIsInStateDir) {
    const configPathHash = crypto.createHash("sha256").update(resolvedConfigPath).digest("hex");
    const preferred = path.join(stateDir, `config.${configPathHash}.last-good`);
    const legacy = [
      path.join(stateDir, `${path.basename(configPath)}.last-good`),
      `${configPath}.last-good`,
    ].filter(
      (candidate, index, candidates) =>
        candidate !== preferred && candidates.indexOf(candidate) === index,
    );
    return { preferred, legacy };
  }

  return { preferred: `${configPath}.last-good`, legacy: [] };
}
...
function resolveLastKnownGoodConfigPath(
  ...
  return resolveLastKnownGoodConfigPaths(configPath, options).preferred;
}
...
export async function recoverConfigFromLastKnownGood(params: {
  ...
  const lastGoodPaths = resolveLastKnownGoodConfigPaths(snapshot.path, {
    env: deps.env,
    homedir: deps.homedir,
  });
  let lastGoodPath = lastGoodPaths.preferred;
  let backupRaw: string | null = null;
  for (const candidatePath of [lastGoodPaths.preferred, ...lastGoodPaths.legacy]) {
    const candidateRaw = await deps.fs.promises.readFile(candidatePath, "utf-8").catch(() => null);
    if (candidateRaw && hashConfigRaw(candidateRaw) === promoted.hash) {
      lastGoodPath = candidatePath;
      backupRaw = candidateRaw;
      break;
    }
  }
  if (!backupRaw) {
  ...
  if (lastGoodPath !== lastGoodPaths.preferred) {
    try {
      await deps.fs.promises.mkdir(path.dirname(lastGoodPaths.preferred), {
        recursive: true,
        mode: 0o700,
      });
      await deps.fs.promises.writeFile(lastGoodPaths.preferred, backupRaw, {
        encoding: "utf-8",
        mode: 0o600,
      });
      await chmodConfigBestEffort({
        deps,
        configPath: lastGoodPaths.preferred,
        context: "last-known-good migration",
      });
    } catch {
      deps.logger.warn(
        "Config last-known-good migration failed; recovery will continue from the authenticated legacy snapshot",
      );
    }
  }
  const now = new Date().toISOString();
