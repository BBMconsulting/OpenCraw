...
describe("doctor SQLite session transcript header repair", () => {
  ...
  it("detects headers through the legacy sessions table before schema migration", async () => {
    await seedHeaderlessTranscript([
      {
        ...
        message: { role: "user", content: "legacy owner table" },
      },
    ]);
    const current = openOpenClawAgentDatabase({ agentId: AGENT_ID, env: state.env });
    const sqlitePath = current.path;
    closeOpenClawAgentDatabasesForTest();

    const legacy = openNodeSqliteDatabase(sqlitePath);
    try {
      legacy.exec(`
    ...
    const inspected = openNodeSqliteDatabase(sqlitePath, { readOnly: true });
    try {
      expect(
        inspected
          .prepare(
            "SELECT name FROM sqlite_schema WHERE type = 'table' AND name IN ('session_windows', 'sessions') ORDER BY name",
          )
          .all(),
