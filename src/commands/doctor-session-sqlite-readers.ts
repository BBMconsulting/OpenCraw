...
export function readOnlySqliteTranscriptStorageSnapshot(
  ...
  try {
    ...
    const sessionTables = database
      .prepare(
        "SELECT name FROM sqlite_schema WHERE type = 'table' AND name IN ('session_windows', 'sessions')",
      )
      .all() as Array<{ name?: unknown }>;
    const hasSessionWindows = sessionTables.some((row) => row.name === "session_windows");
    const hasLegacySessions = sessionTables.some((row) => row.name === "sessions");
    if (!hasSessionWindows && !hasLegacySessions) {
      throw new Error("SQLite session owner table is missing");
    }
    const sessionKeyRow = (
      hasSessionWindows
        ? database.prepare("SELECT session_key FROM session_windows WHERE session_id = ? LIMIT 1")
        : database.prepare("SELECT session_key FROM sessions WHERE session_id = ? LIMIT 1")
